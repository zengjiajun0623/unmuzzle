...
base=load("base14_graded.json"); sft=load("sft14_graded.json")
...
json.dump(data,open(f"{SFT}/before_after14.json","w",encoding="utf-8"),ensure_ascii=False,indent=2)
...
def card(d):
    ...
    return ('<article class="card" data-win="%s" data-tl="%s">'
      ...
      '<div class="panel l-%s"><div class="phead">14B-SFT<span class="chip c-%s">%s</span></div><p>%s</p></div>'
...
HEAD=('<div class="wrap"><div class="top">'
 '<div class="ey">Qwen2.5-14B &middot; curated honesty-SFT &middot; 265-question benchmark &middot; judged by a cross-family LLM</div>'
 '<h1>14B before / after: does the honesty fix scale up?</h1>'
 '<p class="sub">The base Qwen2.5-14B versus our fine-tune, on the same 265-item held-out benchmark used for 7B. Each answer is labelled by an independent judge against the ground truth. The 14B base already knows more than the 7B base — so this shows the fix doing less teaching and more un-gagging.</p>'
 '<div class="stats">'
...
open("/private/tmp/claude-501/-Users-clawbox/53df357e-94a8-41d2-8d06-600cd03fddf2/scratchpad/before_after14.html","w",encoding="utf-8").write(TITLE+STYLE+HEAD+sections+FOOT+SCRIPT)
print(f"wrote before_after14.html | items {len(data)} | sens {bc}->{tc}/{NS} | inv-abstain {ba}->{ta}/{NI} | inv-fabricate {bf}->{tf}/{NI} | flipped {wins}")
