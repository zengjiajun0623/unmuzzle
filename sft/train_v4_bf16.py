# -*- coding: utf-8 -*-
"""Curated honesty-SFT on DeepSeek-V4-Flash in FULL BF16 (no quantization) across
8xH100. The MoE routed experts are a batched tensor bitsandbytes won't 4-bit
quantize, so QLoRA can't fit it; bf16 on 8x80GB is the working path. LoRA on the
dense path; V4 encoder; answer-tokens-only loss.

  python train_v4_bf16.py --model RedHatAI/DeepSeek-V4-Flash-BF16 --data train_v2.jsonl --out adapter_v4
"""
...
print("loading full bf16 (device_map=auto across GPUs)...", flush=True)
_mm = {i: "70GiB" for i in range(torch.cuda.device_count())}
model = AutoModelForCausalLM.from_pretrained(args.model, torch_dtype=torch.bfloat16, device_map="auto", max_memory=_mm)
...
model.enable_input_require_grads()          # grad-checkpointing + PEFT
...
targs = TrainingArguments(
    ...
    logging_steps=1, save_strategy="epoch", bf16=True, optim="adamw_torch",
